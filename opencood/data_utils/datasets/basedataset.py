...
class BaseDataset(Dataset):
    ...
    load_feature: bool
        If set true, instead of loading image/lidar, it will load npy files.
    ...
    def __init__(self, params, visualize, train=True, load_feature=False):
        self.params = params
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # used to save intermediate feature saving
            data[cav_id]['scene'] = \
            cav_content[timestamp_key]['yaml'].split('/')[-3]
            data[cav_id]['origin_yaml'] = \
            ...
            if self.load_feature:
                start = time.time()
                npy_obj = np.load(cav_content[timestamp_key_delay]['npy'])
                data[cav_id]['npy'] = \
                    npy_obj.f.arr_0
                # print("loading caused %f s" % (time.time()-start))
            else:
    ...
    def reinitialize(self):
        print('reinitialzing dataset for augmentation with seed %d' %
              self.seed)
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            if False:
                ...
                random.Random(self.seed).shuffle(cav_list)
                self.seed += 1
            else:
                ...
                for timestamp in timestamps:
                    ...
                    if not self.load_feature:
                        lidar_file = os.path.join(cav_path,
                                                  timestamp + '.pcd')
                        camera_files = self.load_camera_files(cav_path,
                                                              timestamp)
                        ...
                        npy_file = os.path.join(cav_path,
                                                timestamp + '.npz')
                        self.scenario_database[i][cav_id][timestamp]['npy'] = \
                            npy_file
